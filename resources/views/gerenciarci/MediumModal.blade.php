    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                  ...
                  <p>  Tem certeza que deseja apagar a C.I<strong> {{$rs->numero_ci}}</strong> do Solicitante: <strong>{{ $rs->Unidades->nome_unidade }}</strong> ?</p>
                   <p>  Essa operação não poderá ser desfeita. </p>
                
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                <form action="{{ route('gerenciarci.destroy', $rs->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
